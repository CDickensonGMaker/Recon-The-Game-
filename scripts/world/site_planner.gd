...
## THE DUCKBOARDS (his "more ambient floorboard paths", 2026-09-14): `fb_duckboard` sections
## stamped from `data/site_plans/fsb_main_duckboards.json` - generated by
## tools/gen_duckboard_plan.py off the compound's door markers (three boards out from every
## door, and the chow-hall approach) - under the seated compound, like the wire. Soft, no
## destructible, no group: a board is a thing to walk on. A kit without the part stamps nothing
## and says so once; it is not an error.
const FSB_DUCKBOARD_PLAN: String = "fsb_main_duckboards"
const FSB_DUCKBOARD_PART: String = "fb_duckboard"


func _stamp_duckboards(compound: Node3D, center: Vector3) -> void:
	var reg: KitRegistry = KitRegistry.load_kit()
	if reg == null or not reg.has_model(FSB_DUCKBOARD_PART):
		print("[FSB] duckboards: kit part '%s' has no model - no walkways stamped" % FSB_DUCKBOARD_PART)
		return
	var plan: SitePlan = SitePlan.load_from(FSB_DUCKBOARD_PLAN)
	if plan == null:
		print("[FSB] duckboards: no plan '%s' - no walkways stamped" % FSB_DUCKBOARD_PLAN)
		return
	var site: Dictionary = stamp_site_plan(plan, center, reg, compound)
	if site.is_empty():
		push_warning("[FSB] the duckboard plan refused to stamp")
		return
	print("[FSB] duckboards: %d section(s) stamped" % plan.parts.size())
...
func place_firebase_main(center: Vector3) -> Dictionary:
	...
	_stamp_duckboards(compound, center)
