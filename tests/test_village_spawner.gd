...
func _init() -> void:
	...
	var no_firebases: Array = []
	var v_no := load("res://terrain/world/village_spawner.gd").sample_world(42, 1280.0, no_firebases)
	...
	var v2 := load("res://terrain/world/village_spawner.gd").sample_world(42, 1280.0, no_firebases)
	...
	if same:
		for i in v_no.size():
			if v_no[i].world_position != v2[i].world_position:
				same = false
				break
	print("Determinism (seed 42 vs seed 42): %s" % ("PASS" if same else "FAIL"))

	# Different seed should differ
	var v3 := load("res://terrain/world/village_spawner.gd").sample_world(99, 1280.0, no_firebases)
	print("Seed 99 villages: %d (should differ from seed 42)" % v3.size())

	# With a firebase at map center
	var firebases: Array = [Vector2(640.0, 640.0)]
	var v4 := load("res://terrain/world/village_spawner.gd").sample_world(42, 1280.0, firebases)
	print("Seed 42 with 1 firebase at center: %d villages" % v4.size())
	for v in v4:
		print("  %s pos=%s (distance to fb: %.0fm)" % [
