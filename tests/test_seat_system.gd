...
## Instantiates the real huey.tscn (no world needed) and proves the 10-soul
## contract end to end:
##   1. All 10 seat_* sockets exist (auto-generated today; the probe stays
...
func _run() -> void:
	...
	# --- 2. seat 10 mocks (models on a pilot, a gunner, two pax; rest bare)
	...
	if not seats.is_full():
		_fail("10 seated but not is_full")
	...
	if not seats.is_full() and seats.available_seats().size() != 10:
		_fail("after unseat_all, %d seats available (want 10)" % seats.available_seats().size())
	print("  unseat: all 10 restored")
	...
	var aboard_early: int = 10 - seats.available_seats().size()
	await get_tree().create_timer(1.5).timeout  # all beats done
	var aboard: int = 10 - seats.available_seats().size()
	...
	if failures == 0:
		print("PASS: seat system (10-socket contract, clips, stagger, restore) OK")
		get_tree().quit(0)
	else:
