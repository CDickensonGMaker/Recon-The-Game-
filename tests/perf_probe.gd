...
## `viewport_get_measured_render_time_cpu` is the RENDER THREAD's CPU time, NOT the game
## thread. It was labelled "cpu" until 2026-09-08, which meant the game thread - process +
## physics, where the AI lives - was never in a single perf row. Both are sampled now.
var _render_ms: Dictionary = {}  ## phase -> Array[float], render thread
var _game_ms: Dictionary = {}    ## phase -> Array[float], process + physics_process
...
func attach(w: GameWorld) -> void:
	...
	for p: String in _phases:
		...
		_render_ms[p] = [] as Array[float]
		_game_ms[p] = [] as Array[float]
...
func _process(delta: float) -> void:
	...
	if vp2 != null:
		...
		(_render_ms[phase_name] as Array).append(
			RenderingServer.viewport_get_measured_render_time_cpu(rid))
		(_game_ms[phase_name] as Array).append(
...
func _finish() -> void:
	...
	for p: String in _phases:
		...
		print("PERF MS  %-14s gpu_ms_avg=%5.2f gpu_ms_max=%5.2f render_thread_ms_avg=%5.2f render_thread_ms_max=%5.2f game_ms_avg=%5.2f game_ms_max=%5.2f" % [
			p, _avg(_gpu_ms[p]), _maximum(_gpu_ms[p]),
			_avg(_render_ms[p]), _maximum(_render_ms[p]),
			_avg(_game_ms[p]), _maximum(_game_ms[p])])
		if siege_study:
