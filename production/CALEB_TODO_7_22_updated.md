...
**CORRECTION, later the same evening (eleven more census runs, one column each - production/war_room/2026-09-13_npc_first_batch/results.md has the table):** the men who would not leave their quarters were not short of a route. They were walking INTO COTS, LOCKERS, THE RADIO CHAIR, THE CHOW-HALL TENT FRAME AND THE MORTAR-PIT LIP: the navmesh is not carved for the interior furniture, so their route runs straight through it, the body cannot, and the unstick read the velocity the collision had just cancelled as "does not want to move". It now reads the speed the mover asked for, so a man walking into furniture sidesteps after a second: stuck rows 53 -> 26 on the same seed. Also shipped: the agent path points brought down to the feet (the mesh floats a mean 0.42 m above the floor) and every teleport seated on the physics floor like the spawner.
...
- SANDBAGS REBUILT (2026-09-14 afternoon): the ring part is now a 2.28 m window cut from YOUR OWN baked wall (fb_sbg_seg_064 in firebase_v3.2.blend), 567 tris, a 12-tri box collider, one material, 104 KB texture, same footprint so the 224-part plan did not move; the joints overlap 0.28 m so the ring is continuous. Renders: production/renders_firebase/sandbag_segment_single.png and _row.png. YOUR CALL: the look in-game, and the 2 m repeat rhythm along the top course (224 copies of one segment; the old ring had 80 unique walls).

## THE 9/14 EVENING WAVE - the slick goes in, HQ and Poteet both want the new man, and a tool that shows the men thinking (2026-09-14)

Council: production/war_room/2026-09-14_rpg_dealer_hq_map/ (briefing has your words verbatim; synthesis.md is the decree; study_large_maps.md is the map study you asked for). Branches: art + base-sim + perf + audio on BaseGame-V1 and merged forward; the RPG state (crash chain, taskings, dealer, beats, observatory) on RPG-build only. Both pushed.

**What shipped (each gated headlessly; nothing here claims an fps):**
- FIREBASE ART: the ring is your baked sandbag wall again (cut from fb_sbg_seg_064); the TOC is a real bunker you can walk into (the old one's floor was buried under the mound - it was never enterable); the gate sits on the mound with two timber leaves; 170 duckboard sections run out from every door. Renders in production/renders_firebase/. YOUR EYE: the TOC interior, the gate leaves, the 2 m rhythm of the ring's top course. Bunkers are yours by hand.
- THE SLICK GOES IN (your ruling): the 11:30 pad-cycle Huey takes the camp ZPU's fire on an authored inbound leg, trails fire, goes in ~250 m from the wire (huey_crashed.glb, fires, smoke column). The roll is seeded per mission: pilots 1-2 of 2, pax 1-4 of the stick, wounded among the living (HIT 0.6 pace / CRIT 0.35). The living are the ship's OWN men (HeliLift's aircrew and stick, promoted in place - nobody is minted), they FOLLOW you in a file at the slowest man's pace, HOLD if you outrun them past 60 m, and stand down into the garrison (patient / off-duty) inside the wire; the dead lie at the wreck and go into the bill. Radio in period voice (PINK PANTHER / GRAPE / DUSTOFF). Gate: --crash-probe PASS at 512 (4 walked in, 3 in bags; ranges hold). HONEST: the walk home needed the squad's own unseen catch-up snap for men stuck on the berm/wire outside the navmesh - the same nav class as the squad's gate order. YOUR CALL: leave the man behind and the card says so - the mercy fork is real.
- HQ CHAIN: hq/eyes_on (at the gate order: walk the ville and out again with no fire/civ deed - quiet ville: the elder puts a mark on your sheet; wary: nothing; hostile: the responders come), hq/downed_bird (the crash issues it; closes when the men walk in), hq/before_dark (at dusk; the seam settles it). The journal's ORDERS page lists them under WHAT THEY TOLD ME, struck when settled. The assault never waits on any of it.
- THE DEALER (a THAW of your FROZEN "RPG shop", by your words today): SSG Virgil Poteet at the supply dump, [F] like the armorer's bench, moves only what a man eats (never a weapon in either direction - "I DON'T MOVE IRON"). Two asks: walk a case down to the ville (a trade/<ville> deed the ledger holds as its own subject, and the same case turns up in the VC cache) and the slick's mail sack (Poteet or the TOC - one object, two claimants; whichever you serve refuses the other). Prices never move; the rack does. Probe: 62/62.
- BEATS on the existing ScriptedSequence/MissionTrigger runner (data/beats/*.json): THE BARRELS (06:30-07:30 at the burn pad: the detail NCO claims you, Poteet names a price) and THE WIRE AT DUSK ("I SWEAR I SAW SKELETONS OUT IN THE BUSH." - Issue 1 p16, never with a probe man inside the wire). Force-fired in the probe; a natural walk into them is yours. THE CROSSING (the informer with a face, Issue 3 pp13-14) waits on the stream.
- THE OBSERVATORY: F9 in the running game (debug builds), F7 free camera, F6 MAN/LEDGER pane, Tab cycles, click picks. Per man: state and the transition that put him there with its cause, tier, NEAR/FAR/ASLEEP ring, what he last heard (your steps included), last-known, sight cone, witness arrows, schedule, the STUCK reason in the census's own words, a 10-line decision ring. Ledger pane: every ville's band with the deeds and causes behind it, the taskings. Zero cost when off. tools/observatory.tscn boots the demo world with it on. Map pane + time-scrub are follow-ups.
- JUNGLE AUDIBILITY (both branches): steps audible at 48 m walking / 64 m sprinting (was 28), a 10-voice pool with nearest-wins, one occlusion ray at voice start (muffled behind cover), canopy attenuation, a Steps bus with a low-pass, enemy/ally steps as a throttled NoiseBus stimulus, and your OWN step sound so you hear how loud you are. YOUR EARS in a window: is a walker audible at 40 m through gunfire? Knobs named in audio_manager.gd.
- THE THIRD RING (perf, both branches): the watchdog that sleeps men past 240 m is now time-sliced (worst step 16 ms at 1024 vs your 58 ms), never sleeps sappers or a fighting man, snaps a sleeping villager to his scheduled post on wake when unseen, and the law that makes it legal (sleep radius > every noise radius) is a test. --demo-map=N exists (perf_walk.bat at --demo-map=1024 vs default is YOUR paired A/B).
- FOUND AND FIXED: every --print-fps run since 9/11 (your perf_walk included) printed dozens of "'mesh' on Nil" SCRIPT ERRORs - the stall sentinel was shoving a marker node in front of the muzzle-flash pool's quads. Script-less nodes are no longer marked.
