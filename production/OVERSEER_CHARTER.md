...
- **Performance:** `rendering_method = forward_plus` (ADR-026 Amendment A, closed to re-litigation). ~~Last sourced bench: **14.0 → 23.1 fps** on the 18v18 stress arena.~~ **ANNOTATED 2026-09-08: that number came from a DRONE camera 6m up (`ps2_perf_probe.gd:16-17`), a view no player will ever have — direction survives, ratio does not.** The 2026-08-07..2026-09-08 measurement window is void besides: both probes printed the project's render scale instead of the viewport's, and the frames were full-resolution while every row said 0.75 (`PERF_LEDGER.md`, ADR-026 Part A.4 defect note). Fixed 2026-09-08. **No gating FPS number is set — perf remains the top systemic risk.**
