...
>
> **Water is not art at all — it is HYDROLOGY** (§5), and it already generates a real drainage network.
> **For what your EXPORT must actually do for the E&E loop, jump straight to §9.** (Spoiler: three things,
> and two of them are one line each.)
...
## 5 · WATER IS NOT ART. It is HYDROLOGY — and it already works.

**I got this wrong first time** and wrote that the game had no creeks, because I grepped the *vegetation
patch* generator and found none. Water is not a patch. It is generated by `terrain/water/`:

**Priority-Flood pit filling (Barnes et al. 2014) → D8 flow direction → flow accumulation → creeks (<6m)
and rivers (6–50m), plus ponds, lakes and swamps.** Rain lands on the high ground and cascades downhill.
It is a real connected drainage network derived from the terrain — **strictly better than any 12m tile
could be**, because a tiled creek would never connect into anything.

**That network is the E&E network**, because wading breaks the enemy's breadcrumb trail
(`EnemySquad.report_contact(..., leaves_trail)`, shipped 2026-07-12).

### Two things WERE broken in it, and both are now fixed (2026-07-12, `probe_riparian` 7/7)

1. **The banks were not green.** `_determine_terrain_type()` reads elevation and slope only — and a creek
   valley is low and flat, so its banks could classify as RICE_PADDY (density 0.2). `_apply_riparian_belt()`
   now grows **gallery forest** out from every watercourse: **bank density 0.90 → they see you at 55m.**

2. **The water itself had density 0.0** — a **140m firing lane** cut straight through the best cover on the
   map, which is also the *one escape route in the game*. `_roof_the_creeks()` re-scores every water cell by
   **how narrow it is**: a 4m creek between two walls of jungle is **roofed** (you wade a green tunnel); a
   40m river is **open sky** (you are naked in it).

**Still open:** nothing is flagged as an **LZ**. The bird has no hole in the canopy to drop into, and
sprinting for one with the net closing is the climax of the whole run. See §9(3).
