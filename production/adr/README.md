...
- **ADR-042** - [A name is not a contract: the prefix-match bug class](ADR-042-the-naming-contract-bug-class.md) - ACCEPTED. Name-prefix matching is an open set pretending to be closed; it fails silently, toward the wrong default, later, and invisibly to tests. Every name-matching system must enumerate what it did NOT match. Found via two defects in one night: stacked helmets and a possibly-bulletproof firebase.
