...
> **POINTER CORRECTION, 2026-07-23 — ruling unchanged.** `RECON_ADAPTATION.md` and
> `MISSION_DESIGN_RESEARCH.md` (cited at `:2`) were **deleted on purpose by the Summoner**. The `:2`
> citation only claims hunger was *absent* from them, so the ruling is untouched — but do not restore
> those files or go looking for them.
